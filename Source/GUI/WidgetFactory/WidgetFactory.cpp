...
namespace GUI
{
	...
	Widget* WidgetFactory::CreateButton(Widget* aWidget)
	{
		...
		if (widgetName.Find("Quit") != CU::DynamicString::FoundNone)
		{
			...
			return new ButtonDecorator(popStateMessage, aWidget->GetWorldPosition(), aWidget->GetSize(), aWidget);
		}
		else if (widgetName.Find("Start") != CU::DynamicString::FoundNone)
		{
			...
			return new ButtonDecorator(pushPlayStateMessage, aWidget->GetWorldPosition(), aWidget->GetSize(), aWidget);
		}
		else if (widgetName.Find("Credits") != CU::DynamicString::FoundNone)
		{
			...
			return new ButtonDecorator(pushCreditStateMessage, aWidget->GetWorldPosition(), aWidget->GetSize(), aWidget);
		}
		...
		else if (widgetName.Find("Level1") != widgetName.FoundNone)
		{
			...
			return new ButtonDecorator(pushLevelMessage, aWidget->GetWorldPosition(), aWidget->GetSize(), aWidget);
		}
