    ...
    {
        // float4 outputColor;
        // float4 diffuse = diffuseTexture.Sample(Sampler, input.uv.xy);
        // float3 normal = normalMap.Sample(Sampler, input.uv.xy).xyz;
        // normal = (normal * 2.0f) - float3(1.0f, 1.f, 1.f);
        // normal = normalize(normal);

        // float3x3 tangentSpaceMatrix = float3x3(normalize(input.biTangent.xyz), normalize(input.tangent.xyz), normalize(input.normals.xyz)); // om fel, kanske här?

        // normal = mul(normal, tangentSpaceMatrix);

        // float3 ambientDiffuse = diffuse.rgb * 0.01f;
        // float3 directionDiffuse = diffuse.xyz;
        // float lambert = saturate(dot(normal, -myDirectionalLight.direction.xyz));
        // directionDiffuse *= myDirectionalLight.color.rgb * lambert.xxx;


    	// for (int i = 0; i < 8; ++i)
    	// {
    	// 	if (myPointLights[i].intensity > 0.0f)
    	// 	{
    	// 		float3 difference = (myPointLights[i].position.xyz) - input.worldPosition.xyz;
    	// 		float dist = length(difference);
    	// 		float3 direction = normalize(difference);

    	// 		float attenuation = saturate(1.0f - (dist / myPointLights[i].range));
    	// 		lambert = saturate(dot(normal, direction.xyz));
    	// 		directionDiffuse += myPointLights[i].color.rgb * lambert.xxx * attenuation.xxx * myPointLights[i].intensity;
    	// 	}
    	// }

        //outputColor = float4(directionDiffuse, 1.0f );
        //return outputColor;
        PixelOutput output;
